  ...
  return (
    <div
      className={`relative bg-white border rounded p-5 transition-colors ${
        address.isDefault ? "border-[#C8A96B]" : "border-[#e5e5e5]"
      }`}
    >
      {address.isDefault && (
        <span className="absolute top-3 right-3 text-[10px] tracking-[0.12em] uppercase text-[#C8A96B] bg-[#C8A96B]/10 px-2 py-0.5 rounded">
          Default
        </span>
      )}

      <p className="text-sm font-medium text-[#111111]">{address.name}</p>
      <p className="text-xs text-[#8b7355] mt-1">{address.phone}</p>
        ...
        {!address.isDefault && (
          <>
            ...
            <button
              ...
              disabled={isPending}
              className="text-xs text-[#8b7355] hover:text-[#111111] transition-colors disabled:opacity-50"
            >
              Set as default
            </button>
          </>
  ...
  return (
    <div className="space-y-4">
      {addresses.length === 0 && !showAddForm && (
        <p className="text-sm text-[#8b7355]">No saved addresses yet.</p>
      )}

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {addresses.map((addr) =>
          editingAddress?.id === addr.id ? (
            <div key={addr.id} className="bg-white border border-[#C8A96B] rounded p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  ...
                  className="text-xs text-[#8b7355] hover:text-[#111111]"
                >
                  Cancel
                </button>
              </div>
      ...
      {showAddForm ? (
        ...
        <button
          ...
          className="flex items-center gap-2 text-xs text-[#8b7355] hover:text-[#C8A96B] transition-colors"
        >
